...
`--extras ''` for the base CLI. Without `web`, the base CLI and `enso web status|stop` still
work; `enso web start` explains the missing feature. A local installation can save its
future HTTPS update source with `--feed URL`; otherwise it keeps the manifest source.
...
The read-only [web viewer](web.md) is its own process: it runs and restarts independently
of the service, does not survive a reboot (start it again), and shows the same doctor
report as `enso doctor` on its Health page even when `config.json` is broken.
...
migrations can change, selects the candidate, migrates and refreshes eligible bundled content,
...
viewer stays stopped. Cloud can identify its viewer's user service with `--viewer-service NAME`
during installation; the updater checks that it owns this home's viewer before stopping it.

Managed self-updates support the user services generated by `enso service install` and
Enso Cloud. Keep their service definitions and any overrides unchanged until the update or
recovery finishes; concurrent service reconfiguration is unsupported.
