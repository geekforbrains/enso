...
database; maintained knowledge stays in Markdown.
...
6. The message identity is checked for duplicates using the
   [connection rules](connections.md#access-in-020). Attachments download into
   `<workspace>/uploads/<id>/` under [names the transport
...
8. The agent triple resolves: conversation selection, workspace override, else `defaults`.
