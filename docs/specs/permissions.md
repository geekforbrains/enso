...
A missing, unreadable, malformed, or structurally unsafe native policy disables that provider for the affected policy. A missing or unsafe canonical shared instruction source likewise fails `enso config check` and every provider launch closed. If the effective provider is policy-allowed but native-unusable, a Telegram or Slack provider turn (or compaction) receives a configuration error and no provider process starts; Enso does not silently substitute another provider. Non-launch repair commands remain available as described above. A stored route provider that the current policy no longer allows is different: it is not an effective selection, so resolution uses that policy's declared default without erasing the stored preference. If a job's explicit provider is unusable, the job fails before prerun or provider execution and records/notifies that failure through the normal job path. Enso never falls back to an unrestricted launch, another policy, another workspace, an implicit cwd, or an unvalidated prompt.
