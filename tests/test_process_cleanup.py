...
async def test_group_disappearing_after_permission_error_is_reaped(monkeypatch):
    process = child()
    monkeypatch.setattr(runtime.os, "getpgid", lambda _pid: process.pid)
    monkeypatch.setattr(runtime.os, "getpgrp", lambda: 654321)
    ...
    await runtime.terminate_process_tree(process, "exited provider", grace=0.1)
...
async def test_live_group_permission_failure_is_not_hidden(monkeypatch, leader_exited):
    ...
    with pytest.raises(PermissionError):
        await runtime.terminate_process_tree(process, "protected provider", grace=0.1)
...
async def test_unsafe_group_signals_only_the_child(monkeypatch, group):
    process = child()
    monkeypatch.setattr(runtime.os, "getpgid", lambda _pid: group)
    monkeypatch.setattr(runtime.os, "getpgrp", lambda: 654321)
    kill = Mock()
    monkeypatch.setattr(runtime.os, "killpg", kill)

    await runtime.terminate_process_tree(process, "shared group provider", grace=0.1)
