...
The same engine answers for trust, finite repair, and retained execution ownership: an
agent cannot buy acceptance by weakening the rules, and an interrupted integration never
double-lands or accepts unchecked work.
"""
...
def configure(
    paths: Paths, config: Config, stages=None, *, checks=(), max_repairs=2, hooks=None, repo=None
):
    fields = {
...
def claim(paths, config, ref, run="r1"):
    ...
    workflows.start(paths, config, ref, run)


def held(paths, config, run="r1"):
    task = tasks.create(paths, config, "EN", "Build a feature", actor="user:test")
    claim(paths, config, task.ref, run)
    return task


def submit(paths, config, ref, run="r1", move="advance"):
    return tasks.move(
        paths, config, ref, move, actor="job:test", run_id=run, message="Candidate ready"
    )
...
async def test_dirty_submissions_consume_the_configured_repair_budget(
    ...
    config = configure(enso_home, project_config, [{"name": "work", "max_repairs": 1}], repo=repo)
...
@pytest.mark.parametrize(
    ("name", "change"),
    [
        # One case per arm of the rule: an existing assertion may not be rewritten or
        # removed, while a new file is ordinary work. Every case builds a real repository.
        ("backend/internal/store/store_test.go", "modify"),
        ("test_health.py", "delete"),
        ("pkg/health_test.py", "add"),
    ],
)
...
async def test_operator_reset_replenishes_exhausted_return_budget(
    ...
    for run_id in ("r1", "r2"):
        ...
        submit(enso_home, config, task.ref, run_id, "return")
    ...
    submit(enso_home, config, task.ref, "r3", "return")
...
def test_dependency_completion_does_not_release_a_blocked_tasks_live_writer(
    ...
    config = configure(enso_home, project_config, ["work"])
...
async def test_lifecycle_missing_project_records_failure_without_changing_owner(
    ...
    config = configure(enso_home, project_config, ["work"], hooks={"after:done": "touch misplaced"})
