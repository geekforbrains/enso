...
def markdown(message: OutboundMessage) -> str:
    """Readable capture representation of rich blocks, including chart labels and values."""

    def table(rows: tuple[tuple[str | int | float, ...], ...]) -> str:
        def row(cells: tuple[str | int | float, ...]) -> str:
            return (
                "| "
        ...
        return "\n".join(
            [row(rows[0]), row(tuple("---" for _ in rows[0])), *(row(cells) for cells in rows[1:])]
        )
    ...
    for block in message.blocks:
        if isinstance(block, MarkdownBlock):
            ...
            rows: tuple[tuple[str | int | float, ...], ...]
            if block.kind == "pie":
                rows = (("Label", "Value"), *((s.label, s.value) for s in block.segments))
            else:
                rows = (
                    (block.x_label or "Category", *(s.name for s in block.series)),
                    *(
                        (label, *(s.data[i] for s in block.series))
                        for i, label in enumerate(block.categories)
                    ),
                )
            caption = block.title + (f" ({block.y_label})" if block.y_label else "")
